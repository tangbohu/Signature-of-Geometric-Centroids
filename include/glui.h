/****************************************************************************
  ...
  Modified by Philip Fu in 2001, Version 2.02f
  - slider class added
...
#include <GLUT/glut.h>				/* for GLUT, it includes GL */
#else						/* GL/ on IRIX for GLUT and GL */
#include <GL/glut.h>				/* for GL and such */
#endif
...
#define GLUI_VERSION 2.02f    /********** Current version **********/
...
/*** Codes for different type of edittext boxes and spinners and sliders ***/
...
#define GLUI_SLIDER_INT                GLUI_EDITTEXT_INT
#define GLUI_SLIDER_FLOAT              GLUI_EDITTEXT_FLOAT
...
public:
  ...
  void  unlink( int parent_and_sibling_only = false );
  int   get_num_childs(int of_collapsed_node=false);
...
enum GLUI_StdBitmaps_Codes {
  ...
  GLUI_STDBITMAP_SLIDER_RIGHT_OFF,
  GLUI_STDBITMAP_SLIDER_RIGHT_ON,
  GLUI_STDBITMAP_SLIDER_LEFT_OFF,
  GLUI_STDBITMAP_SLIDER_LEFT_ON,
  GLUI_STDBITMAP_SLIDER_RIGHT_DIS,
  GLUI_STDBITMAP_SLIDER_LEFT_DIS,
  GLUI_STDBITMAP_SLIDER_TAB,
  GLUI_STDBITMAP_SLIDER_TAB_DIS,
...
extern int glui_img_sliderright_0[];
extern int glui_img_sliderright_1[];
extern int glui_img_sliderleft_0[];
extern int glui_img_sliderleft_1[];
extern int glui_img_sliderright_dis[];
extern int glui_img_sliderleft_dis[];
extern int glui_img_slidertab[];
extern int glui_img_slidertab_dis[];
...
protected:
  ...
  */
  ...
  friend int     glui_parent_window_keyboard_func( unsigned char, int, int );
...
public:
  ...
  // add/remove controls (allows us to change the panels during runtime)
  int    add_control( GLUI_Node *parent, GLUI_Control *control, int re_add=false );
  int    remove_control( GLUI_Node *parent, GLUI_Control *control, int need_refresh=true );
  int    remove_control( GLUI_Node *parent, int childIndex, int need_refresh=true );
  ...
  RGBc             fogd_color;
  float            fogd_color_f[3];
  RGBc             disable_color;
  float            disable_color_f[3];
...
public:
  ...
  int             fixedW;			/* allows user to change? */
  int             x_abs, y_abs;
  ...
  // Philip: for dynamic control management
  GLUI_Node       temp_node;
  ...
  virtual void disactivate( int force=false )	{ active = false; };
  ...
  void         set_to_fogd_color( void );
  void         set_to_disable_color( void );

  void         set_w( int new_w, int new_fixedW=false );
...
public:
  ...
  void  add_separator( int draw_bar = true, int sep_height = GLUI_SEPARATOR_HEIGHT );
  void  add_separator_to_panel( GLUI_Panel *panel, int draw_bar = true, int sep_height = GLUI_SEPARATOR_HEIGHT );
  ...
  GLUI_EditText  *add_edittext( char *name, 
				...
				void *live_var=NULL,
				int id=-1, GLUI_Update_CB callback=NULL, int in_activiateCBwhenEnter=false );
  GLUI_EditText  *add_edittext_to_panel( GLUI_Panel *panel, 
					 char *name,
					 int data_type=GLUI_EDITTEXT_TEXT,
					 void *live_var=NULL, int id=-1, 
					 GLUI_Update_CB callback=NULL, int in_activiateCBwhenEnter=false );
  ...
  // overload float and int

  GLUI_Slider  *add_slider( char *name, 
                            int data_type=GLUI_SLIDER_FLOAT, void *data=NULL, 
                            float low=-1.0f, float high=1.0f, 
                            int id=-1, GLUI_Update_CB callback=NULL );
  GLUI_Slider  *add_slider( char *name, 
                            int data_type=GLUI_SLIDER_INT, void *data=NULL, 
  ...
  GLUI_HSlider  *add_hslider( char *name=NULL,
                              int resizable=false,
                              int data_type=GLUI_HSLIDER_INT,
                              void *live_var=NULL,
                              int id=-1, GLUI_Update_CB callback=NULL );

  GLUI_HSlider  *add_hslider_to_panel( GLUI_Panel *panel,
                                       char *name=NULL, 
                                       int resizable=false,
                                       int data_type=GLUI_HSLIDER_INT,
                                       void *live_var=NULL,
...
private :
  GLUI_Slider  *actual_add_slider_to_panel( GLUI_Panel *panel, char *name, 
...
public:
  ...
  int                 activiateCBwhenEnter;	/* Philip : true => activate CB when user hit enter only */
  ...
  void disactivate( int force=false );
...
#define  GLUI_SLIDER_ARROW_WIDTH	8
#define  GLUI_SLIDER_ARROW_HEIGHT	12
#define  GLUI_SLIDER_ARROW_Y		2

#define  GLUI_SLIDER_STATE_NONE		0
#define  GLUI_SLIDER_STATE_RIGHT	1
#define  GLUI_SLIDER_STATE_LEFT		2
#define  GLUI_SLIDER_STATE_TAB		4
#define  GLUI_SLIDER_STATE_RIGHT_TAB	5
#define  GLUI_SLIDER_STATE_LEFT_TAB	6

#define  GLUI_SLIDER_DEFAULT_GROWTH_EXP   1.05f
...
public:
  int           currently_inside;
  int           state;
  int           mouse_x, mouse_y, tab_relative_x;	// tab_relative_x used when user click on "tab"
							// it remembers the x difference between mouse 
							// and tab starting x at the clicking time
  float         growth, growth_exp;
  int           last_x, last_y;
  int           data_type;
  int           callback_count;
  ...
  int  find_slider( int local_x, int local_y );
  int  get_tab_x();
  void set_tab_x(int pos);
  void do_drag( int x, int y );
  void do_callbacks( void );
  void draw_arrows( void );
  void do_click( void );
  ...
  GLUI_Slider( void ) {
    ...
    h              = (GLUI_EDITTEXT_HEIGHT+GLUI_SLIDER_ARROW_HEIGHT);
    w              = GLUI_EDITTEXT_WIDTH;
    x_off          = 0;
    y_off_top      = 0;
    y_off_bot      = 0;
    can_activate   = true;
    state          = GLUI_SLIDER_STATE_NONE;
    mouse_x        = -1;
    mouse_y        = -1;
    tab_relative_x = 0;
    edittext       = NULL;
    growth_exp     = GLUI_SLIDER_DEFAULT_GROWTH_EXP;
    callback_count = 0;
    first_callback = true;
    user_speed     = 1.0;
  };
...
#define  GLUI_HSLIDER_ARROW_WIDTH		GLUI_SLIDER_ARROW_WIDTH
#define  GLUI_HSLIDER_ARROW_HEIGHT		GLUI_SLIDER_ARROW_HEIGHT
...
public:
  ...
  class GLUI_StaticText *staticText;
...
int  glui_parent_window_keyboard_func(unsigned char key, int x, int y);
