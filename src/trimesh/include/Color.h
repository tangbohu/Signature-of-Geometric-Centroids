...
// Defined by Song Peng on 2014.12.04
#ifndef _MAX
...
private:
	...
	const Color srgb2hsv() const
	{
		...
		// Modified by Song Peng to solve following issue:
		// error C2784: 'trimesh::Vec<D,T> std::max(const trimesh::Vec<D,T> &,const T &)' :
		//              could not deduce template argument for 'const trimesh::Vec<D,T> &' from 'const float'
		float V = _MAX(_MAX(v[0], v[1]), v[2]);
		float diff = V - _MAX(_MAX(v[0], v[1]), v[2]);
